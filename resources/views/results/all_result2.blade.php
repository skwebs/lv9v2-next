...
<?php 
  ...
  function res($m){
	...
	foreach($students as $stu){
		echo($stu->result->marks->english).'<hr>';
	}
              ...
              <td>
                <table class="table table-sm marks text-center  mb-0">
                  ...
                  <tr>
                    ...
                    <td>100</td>
                    <td>{{ $stu->result->marks->english }}</td>
                    <td>NA</td>
                    <td>{{ $stu->result->marks->english }}</td>
                    <td>{{ res($stu->result->marks->english)['g'] }}</td>
                  </tr>
                  <tr>
                    <th class="text-start">Hindi</th>
                    <td>100</td>
                    <td>{{ $stu->result->marks->hindi }}</td>
                    <td>NA</td>
                    <td>{{ $stu->result->marks->hindi }}</td>
	                  <td>{{ res($stu->result->marks->hindi)['g'] }}</td>
                  </tr>
                  <tr>
                    <th class="text-start">Maths</th>
                    <td>100</td>
                    <td>{{ $stu->result->marks->maths }}</td>
                    <td>NA</td>
                    <td>{{ $stu->result->marks->maths }}</td>
                  <td>{{ res($stu->result->marks->maths)['g'] }}</td>
                  </tr>
              @if(!in_array($stu->class, $classes))
                  <tr>
                    <th class="text-start">Science</th>
                    <td>100</td>
                    <td>{{ $stu->result->marks->science }}</td>
                    <td>{{ $stu->result->marks->science_oral }}</td>
                    <td>{{ $stu->result->marks->science + $stu->result->marks->science_oral }}</td>
	                  <td>{{ res($stu->result->marks->science + $stu->result->marks->science_oral)['g'] }}</td>
                  </tr>
                  <tr>
                    <th class="text-start">S.St</th>
                    <td>100</td>
                    <td>{{ $stu->result->marks->sst }}</td>
                    <td>{{ $stu->result->marks->sst_oral }}</td>
                    <td>{{ $stu->result->marks->sst + $stu->result->marks->sst_oral }}</td>
                    <td>B+</td>
                  </tr>
                  <tr>
	                  <th class="text-start">Computer</th>
	                  <td>100</td>
	                  <td>{{ $stu->result->marks->computer }}</td>
	                  <td>NA</td>
	                  <td>{{ $stu->result->marks->computer }}</td>
	                  <td>{{ res($stu->result->marks->computer)['g'] }}</td>
	              </tr>
              @endif
                  <tr>
                    <th class="text-start">Total</th>
                    <th>{{ $stu->result->full_marks }}</th>
                    <th>
                    <?php 
                    if(!in_array($stu->class, $classes)){
                    echo $stu->result->total - ( $stu->result->marks->science_oral + $stu->result->marks->sst_oral ) ;
                    }else{
                    echo $stu->result->total;
                    } ?>
                    </th>
                    <th>
                     <?php 
                     if(!in_array($stu->class, $classes)){
                     echo $stu->result->marks->science_oral + $stu->result->marks->sst_oral ;
                     }else{
                     echo "NA";} ?>
                     </th>
                    
                    <th>{{ $stu->result->total }}</th>
                  <td>{{ res(($stu->result->total*100)/$stu->result->full_marks)['g'] }}</td>
                  </tr>
                  <tr>
	                  <td colspan="5" class="text-start ps-5">
		                  <strong>Obtained Marks in words:</strong> {{$stu->result->total_text}}.
	                  </td>
	              </tr>
                  <tr>
	                  <td class="text-start" colspan="6" ><strong>Extra Activity (Drawing)</strong> Marks: <strong>{{$stu->result->marks->drawing}}</strong> &nbsp;| Grade: <strong>{{ res($stu->result->marks->drawing)['g']}}</strong></td>
                  </tr>
                    ...
                    <td width="100%" class="d-flex justify-content-center">
                      <table class="text-center p-2" width="160">
                        <tr class="bg-success text-center fw-bold bg-success text-white">
                          ...
                          <td>:</td>
                          <td class="pe-2 py-2">{{ res(($stu->result->total*100)/$stu->result->full_marks)['g'] }}</td>
                        </tr>
                      </table>
                    </td>
                  </tr>
	  ...
	  <tr>
        <td class="p-2" >
            <table class="table table-sm table-bordered border border-dark text-center" >
                ...
                <tr>
                    <td>{{$stu->result->total}}/{{$stu->result->full_marks}}</td>
                    <td>
	                    <?php
		                    echo round(($stu->result->total*100)/$stu->result->full_marks,2);
	                    ?>%
                    </td>
                    <td>{{ res(($stu->result->total*100)/$stu->result->full_marks)['g'] }}</td>
                    <td>{{ $stu->result->position }}</td>
                </tr>
            </table>
          </td>
      </tr>
